...
#-----------------------------English/Thai news-------------------#
    ...
    def get_eng_news(self): #https://towardsdatascience.com/how-to-get-the-latest-covid-19-news-using-google-news-feed-950d9deb18f1
        ...
        #sentiment
        print('sentiment part')
        data2 = pd.read_csv('eng_news.csv')
        pos = 0
        neg = 0
        neu = 0
        for tweet in data2['Headline']:
            analysis = TextBlob(tweet)
            if analysis.sentiment[0]>0:  #1 is positive
                pos +=  1
            elif analysis.sentiment[0]<0: #-1 is a negative
    ...
    def get_thai_news(self):
        ...
        print('sentiment part')
        data2 = pd.read_csv('thai_news.csv')
        P = self.loadData1() #pickle เรียกไฟล์โหลดมา
        pos = 0
        neg = 0
        neu = 0
        for news in data2['Headline']:
            text = str(news).lower()
            text = re.sub('[^ก-๙]','',text)
            allchars = [str for str in text]
            emoji_list = [c for c in allchars if c in emoji.UNICODE_EMOJI] #ลบ emoji ออก
...
#--------------------ProgressBar----------------------------------------#
    ...
    def doAction(self): #https://www.geeksforgeeks.org/pyqt5-qprogressbar-how-to-create-progress-bar/
