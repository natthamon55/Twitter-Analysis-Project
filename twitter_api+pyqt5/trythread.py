...
class Worker(QObject): #ทำงานกับเทรด #ห้ามยุ่งกับ gui
    progress1 = pyqtSignal(str,str) #การรับค่า
    progress2 = pyqtSignal(str,str)
    progress3 = pyqtSignal(str,str)
    finished = pyqtSignal()
...
#------------------------Twitter---------------------------------------#   
    def twit(self,keyword1,s,e): #real time 100 tweet
        ...
        self.progress1.emit(keyword1,'twitter.csv')
...
#-----------------------------English/Thai news-------------------#
    ...
    def get_eng_news(self,search): #https://towardsdatascience.com/how-to-get-the-latest-covid-19-news-using-google-news-feed-950d9deb18f1
        ...
        self.progress2.emit(search,'eng_news.csv')
    ...
    def get_thai_news(self,search):
        ...
        self.progress2.emit(search,'thai_news.csv')
            ...
            try :
                ...
                if date1 == date2  : #หาแบบ 1 วัน
                    ...
                    if op1 in check: #ถ้าวันกับคำ อยู่ในวันที่ check 
                        ...
                        self.progress2.emit(search,'find_news.csv')
...
#---------------------------stoock--------------------------------------------#
    def search_stock(self,search,s,e):   #ดึงข้อมูลราคาหุ้น จาก https://finance.yahoo.com/portfolios
        ...
        elif search != '' : #หาหุ้น รับวันแบบ 2021-03-4
            df = data.DataReader(search,data_source='yahoo',start = s,end = e)
            df.to_csv('stock.csv')

            self.progress3.emit(search,'stock.csv')
            #plot graph
...
#-----------------Main Search---------------------------------#
    def search_part(self):
        ...
        if self.key1 == '' : #ถ้าไม่ได้หา keyword twit
            ...
            self.check_news(self.key2,self.date1,self.date2)#news
            self.search_stock(self.stock,day1,day2)
            ...
            try:
                ...
                if self.date1 == self.date2  : #หาแบบ 1 วัน
                    ...
                    self.progress1.emit(self.key1,'find_store.csv')
                    #self.hash_tag('find_store.csv') #top hashtag
                    #self.sent_twit(self.key1,'find_store.csv')  #sentiment twitter

                    self.check_news(self.key2,self.date1,self.date2) #news
                    self.search_stock(self.stock,day1,day2) #stock
                ...
                self.twit(self.key1,day1,day2) #twitter realtime
                self.check_news(self.key2,self.date1,self.date2)#news
                self.search_stock(self.stock,day1,day2)
...
#----------------------Search Twit & News & Stock-----------------------------------#
    def search_part(self):
        ...
        self.thread.started.connect(self.worker.search_part)
        self.worker.finished.connect(self.thread.quit)
        self.worker.finished.connect(self.worker.deleteLater)
        self.thread.finished.connect(self.thread.deleteLater)
        self.worker.progress1.connect(self.link1)#twit
        self.worker.progress2.connect(self.link2)#news
        self.worker.progress3.connect(self.link3)#stock
        # Step 6: Start the thread
        self.thread.start()

    def link1(self,key,path): #ใช้วาดรูปกราฟ twit
        #analysis
        self.hash_tag(path)
        self.sent_twit(key,path) #sentiment twitter
    
    def link2(self,key,path): #news
        self.sent_news(key,path)

    def link3(self,key,path): #stock
        self.plot_stock(key,path)
        plt.close('all') #closes all the figure windows
...
#####-----------------------------Sentiment news--------------------------------------------------------#### 
    def sent_news(self,search,path):
