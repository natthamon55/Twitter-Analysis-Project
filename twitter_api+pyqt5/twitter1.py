...
###--------------------------Twitter ----------------------------------###     
    def twit(self,keyword1,s,e): #real time 100 tweet
        ...
        #serch realtime
        self.search_tweet2 = tweepy.Cursor(self.api.search,q=self.keyword,result_type='recent',since=since,until=until,tweet_mode='extended').items(80)
...
#----------------------Search Part-----------------------------------#
    def search_twit(self):
        ...
        date1 = self.dateEdit.text() #calendar
        date2 = self.dateEdit2.text()
...
#####-----------------------------english news--------------------------------------------------------#### 
    ...
    def get_the_news(self,search): #https://www.youtube.com/watch?v=hMxFnVVYGDk
        ...
        df.to_csv('eng_news.csv')
        
        df1 = pd.read_csv('eng_news.csv')
...
###------------------------stock------------------------------------------###
    ...
    def search_stock(self):   #ดึงข้อมูลราคาหุ้น จาก https://finance.yahoo.com/portfolios
        ...
        #plot graph
        self.plot_stock('stock.csv',stock)
