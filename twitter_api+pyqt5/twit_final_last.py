...
#--------------------News EN/TH---------------------------------#
    def check_news(self) :
        ...
        #- - - - - - - - - - - - เริ่มส่วนที่เเก้ - - - - - - - - - - - - - - - - - -
        day_list = []
        date_start = datetime.strptime(self.day1 ,'%Y-%m-%d')
        date_end = datetime.strptime(self.day2 ,'%Y-%m-%d')

        while int((date_end - date_start).days) >= 0 : 
            day_list.append("web_" + date_start.strftime('%Y-%m-%d') + "_.csv")
