...
#------------------------Twitter---------------------------------------#   
    def twit(self): #real time 100 tweet
        ...
        if re.match('[ก-๙]',self.keyword) !=  None:
            ...
            chart = QChart()
            chart.addSeries(se)
            chart.setTitle("Thai-TwitSentiment")
            chartview = QChartView(chart)
            chartview.setGeometry(0,0,600,500)
            chartview.setRenderHint(QPainter.Antialiasing)

            self.savepi = QPixmap(chartview.grab())
            self.savepi.save("C:/software/software2/pic/sent2.png", "PNG")
            ...
            chart = QChart()
            chart.addSeries(se)
            chart.setTitle("Eng-TwitSentiment")
            chartview = QChartView(chart)
            chartview.setGeometry(0,0,600,500)
            chartview.setRenderHint(QPainter.Antialiasing)

            self.savepi = QPixmap(chartview.grab())
            self.savepi.save("C:/software/software2/pic/sent1.png", "PNG")
