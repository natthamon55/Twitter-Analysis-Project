...
###--------------------------Twitter ----------------------------------###     
    def twit(self,keyword1): #real time
        ...
        #serch realtime
        self.search_tweet2 = tweepy.Cursor(self.api.search,q=self.keyword,result_type='recent',tweet_mode='extended').items(80)
    ...
    def str_day(self): #https://www.geeksforgeeks.org/pyqt5-getting-the-text-of-selected-item-in-combobox/
        data = pd.read_csv("keyword.csv",names=['Date', 'Keyword']) #ไฟล์ที่เก็บคำ search
...
#----------------------Search Part-----------------------------------#
    def search_twit(self):
        ...
        date1 = self.date1.currentText() #since '14/03/2021'
        date2 = self.date2.currentText()  # until 
        ...
        if key1 in keyword : #in store
            ...
            try:
                ...
                self.twit(key1) #twitter realtime
                self.find_news(key2)#news
...
#------------------------twitter sentiment---------------------#
    def sent_twit(self,search,path):
        ...
        if re.match('[ก-๙]',search) !=  None:
            ...
            se.append('Neutral 0',int(neu))

            chart = QChart()
            chart.addSeries(se)
            chart.setTitle('Thai-Twit Sentiment')
            ...
            chart = QChart()
            chart.addSeries(se)
            chart.setTitle('ENG-Twit Sentiment')
            ...
            self.saveapi = QPixmap(chartview.grab())
            self.saveapi.save('C:/software/software2/pic/sent1.png','PNG')
...
#####-----------------------------english news--------------------------------------------------------#### 
    ...
    def eng_sent(self,path): #sentiment eng_news
        ...
        self.saveapi = QPixmap(chartview.grab())
        self.saveapi.save('C:/software/software2/pic/ennews_sent.png','PNG')
...
###-------------------- Thai news-----------------------####
    ...
    def thai_sent(self,path):
        ...
        self.saveapi = QPixmap(chartview.grab())
        self.saveapi.save('C:/software/software2/pic/thnews_sent.png','PNG')
...
##--------------------Th/Eng news-------------------------------------##
    def find_news(self,search) :
        ...
        if re.match('[ก-๙]',search) !=  None: #เทียบภาษา
